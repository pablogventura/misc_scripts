...
#Copyright (C) 2009 Robert Wixler
...
import dbus, re, os, SocketServer, socket
port = raw_input("Enter a port for the RhythmBox-Server(press Enter to use the default port 8484):")
if port =="":
	port=8484
else:
	port = int(port)
...
test_socket.close()
print 'Started RhythmBox-Server server on port %s' % (port)
